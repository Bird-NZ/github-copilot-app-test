...
curl -s "$BASE/workspaces/$WS/ir3/calc" | python3 -c 'import sys,json; d=json.load(sys.stdin); assert "calc" in d and "map" in d and d["calc"]["summary"]["donationClaim"] > 50'
curl -s "$BASE/workspaces/$WS/export/draft" | python3 -c 'import sys,json; d=json.load(sys.stdin); assert "csv" in d and "pdf" in d and "json" in d and d["json"]["workspace"]["id"] and d["pdf"]["mimeType"]=="application/pdf" and d["pdf"]["bytesBase64"] and d["review"]["crypto"]["intro"] and any(item["activity"].startswith("Selling crypto") for item in d["review"]["crypto"]["taxableActivities"]) and any("PIE income" in bullet for bullet in d["explanation"]["bullets"]) and any("other tax already deducted" in bullet for bullet in d["explanation"]["bullets"])'
curl -s "$BASE/workspaces/$WS/review" | python3 -c 'import sys,json; d=json.load(sys.stdin); review=d["review"]; supports={(item["supports"], tuple(item["ir3Refs"])) for item in review["evidence"] if item["documentType"] == "crypto_csv"}; crypto_warning=next((warning for warning in review["warnings"] if warning["code"]=="CRYPTO_EVIDENCE_MISSING"), None); student_warning=next((warning for warning in review["warnings"] if warning["code"]=="MISSING_STUDENT_LOAN_DOC"), None); pie_warning=next((warning for warning in review["warnings"] if warning["code"]=="PIE_CREDITS_HIGH_FOR_INCOME"), None); loan_status=review["summary"]["studentLoanStatus"]; assert review["crypto"]["status"]["hasCryptoCsv"] is True and review["crypto"]["transactionCounts"]["buy"] == 1 and "NZD value" in " ".join(review["crypto"]["whatToProvide"]) and ("PAYE income", ("11B", "11C")) in supports and ("Crypto transaction history", ("crypto",)) in supports and crypto_warning is None and student_warning and student_warning["evidenceOverride"]["mode"]=="manual" and student_warning["evidence"][0]["documentId"] and pie_warning and review["summary"]["pieIncome"] == 1000.13 and review["summary"]["pieTaxCredits"] == 351 and review["summary"]["extraTaxDeducted"] == 125.56 and loan_status["hasStudentLoan"] is True and loan_status["hasStatement"] is False and loan_status["status"] == "needs_attention"'
