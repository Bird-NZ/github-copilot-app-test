...
curl -s -X PATCH "$BASE/workspaces/$WS/documents/$DOC_ID/evidence-link" -H 'content-type: application/json' -d '{"evidenceLink":{"mode":"manual","supports":"PAYE income","section":"Income","ir3Refs":["11B","11C"],"summaryKey":null}}' | python3 -c 'import sys,json; d=json.load(sys.stdin); assert d["document"]["evidenceLink"]["mode"]=="manual" and d["document"]["evidenceLink"]["supports"]=="PAYE income"'
...
curl -s "$BASE/workspaces/$WS/review" | python3 -c 'import sys,json; d=json.load(sys.stdin); review=d["review"]; assert review["crypto"]["status"]["hasCryptoCsv"] is True and review["crypto"]["transactionCounts"]["buy"] == 1 and "NZD value" in " ".join(review["crypto"]["whatToProvide"]) and any(item["documentType"] == "crypto_csv" and item["supports"] == "PAYE income" and item["linkMode"] == "manual" and "11B" in item["ir3Refs"] for item in review["evidence"])'
