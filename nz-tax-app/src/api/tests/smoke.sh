...
curl -s "$BASE/workspaces/$WS/review" | python3 -c 'import sys,json; d=json.load(sys.stdin); review=d["review"]; supports={(item["supports"], tuple(item["ir3Refs"])) for item in review["evidence"] if item["documentType"] == "crypto_csv"}; crypto_warning=next((warning for warning in review["warnings"] if warning["code"]=="CRYPTO_EVIDENCE_MISSING"), None); student_warning=next((warning for warning in review["warnings"] if warning["code"]=="MISSING_STUDENT_LOAN_DOC"), None); pie_warning=next((warning for warning in review["warnings"] if warning["code"]=="PIE_CREDITS_HIGH_FOR_INCOME"), None); provisional_warning=next((warning for warning in review["warnings"] if warning["code"]=="PROVISIONAL_TAX_RISK"), None); loan_status=review["summary"]["studentLoanStatus"]; provisional_status=review["summary"]["provisionalTaxStatus"]; submission=review["submissionReadiness"]; blockers={item["code"] for item in submission["blockers"]}; assert review["crypto"]["status"]["hasCryptoCsv"] is True and review["crypto"]["transactionCounts"]["buy"] == 1 and "NZD value" in " ".join(review["crypto"]["whatToProvide"]) and ("PAYE income", ("11B", "11C")) in supports and ("Crypto transaction history", ("crypto",)) in supports and crypto_warning is None and student_warning and student_warning["evidenceOverride"]["mode"]=="manual" and student_warning["evidence"][0]["documentId"] and pie_warning and provisional_warning and "standard option" in provisional_warning["message"].lower() and provisional_status["relevant"] is True and provisional_status["threshold"] == 5000 and provisional_status["estimatedStandardOptionTax"] > provisional_status["modeledResidualIncomeTax"] and review["summary"]["pieIncome"] == 1000.13 and review["summary"]["pieTaxCredits"] == 351 and review["summary"]["extraTaxDeducted"] == 125.56 and loan_status["hasStudentLoan"] is True and loan_status["hasStatement"] is False and loan_status["status"] == "needs_attention" and submission["status"] == "action_needed" and submission["questionnaire"]["complete"] is False and submission["documents"]["applicableCount"] >= 3 and submission["documents"]["receivedCount"] >= 2 and "QUESTIONNAIRE_INCOMPLETE" in blockers and "DOC_STUDENT_LOAN_STATEMENT_MISSING" in blockers and any("questionnaire" in action.lower() for action in submission["nextActions"])'
