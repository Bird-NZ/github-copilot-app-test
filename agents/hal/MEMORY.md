...
- User preference: for prompt-engineering tasks, do iterative testing/refinement until excellence threshold is reached (not fixed to 3 cycles), then return best prompt + evidence.
- User preference: for Copilot prompt work, enforce a 2-layer quality gate: (A) prompt-mechanics optimization and (B) deeper source-verified research before marking production-ready.
...
- User preference: future detailed design docs should match the Battleships detailed design document style/structure as the default template.
- User preference: when building Copilot agents, provide complete field-by-field instructions with no omissions, including example test values/checks for every field.
